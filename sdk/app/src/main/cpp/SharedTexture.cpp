...
std::shared_ptr<SharedTexture> SharedTexture::Make(int width, int height,bool isMultiview) {
    if (!AVAILABLE) {
        XR_ERROR("Make failed: not AVAILABLE");
        return nullptr;
    }
    ...
    AHardwareBuffer_Desc desc = {
            ...
            static_cast<uint32_t>(isMultiview ? 2 : 1),
    ...
    if (errCode != 0 || !buffer) {
        XR_ERROR("Make failed: AHardwareBuffer_allocate error: %d", errCode);
        return nullptr;
    }
    return std::shared_ptr<SharedTexture>(
            new SharedTexture(buffer, static_cast<int>(desc.width), static_cast<int>(desc.height)));
}

std::shared_ptr<SharedTexture> SharedTexture::MakeAdopted(AHardwareBuffer *buffer) {
    if (!AVAILABLE) {
        XR_ERROR("MakeAdopted failed: not AVAILABLE");
        return nullptr;
    }
    if (!buffer) {
        XR_ERROR("MakeAdopted failed: buffer null");
        return nullptr;
    }
    AHardwareBuffer_Desc desc;
    HWDriver::AHardwareBuffer_describe(buffer, &desc);
    HWDriver::AHardwareBuffer_acquire(buffer);
    XR_ERROR("AHardwareBuffe format=%d,layers=%d",desc.format,desc.layers);
    return std::shared_ptr<SharedTexture>(
            new SharedTexture(buffer, static_cast<int>(desc.width), static_cast<int>(desc.height)));
}
...
std::shared_ptr<SharedTexture> SharedTexture::MakeAdoptedJObject(JNIEnv *env, jobject buffer) {
    if (!AVAILABLE) {
        XR_ERROR("MakeAdoptedJObject failed: not AVAILABLE");
        return nullptr;
    }
    if (!buffer) {
        XR_ERROR("MakeAdoptedJObject failed: buffer null");
        return nullptr;
    }
...
SharedTexture::SharedTexture(AHardwareBuffer *buffer, int width, int height)
    ...
    XR_ERROR("SharedTexture:(%p)(%d, %d)",buffer, width, height);
}

SharedTexture::~SharedTexture() {
    XR_ERROR("~SharedTexture");
    ...
    if(bindTexId_){
        GL(glDeleteTextures(1,&bindTexId_));
    }
}

bool SharedTexture::bindTexture(unsigned texId,bool ismultiview) {
    XR_LOGGER_NO_INSTANCE(logger);
    if (!AVAILABLE) {
        XR_ERROR("bindTexture failed: not AVAILABLE");
        return false;
    }

    if (!buffer_ || texId == 0) {
        XR_ERROR("bindTexture failed: buffer_ or texId null");
        return false;
    }

    EGLClientBuffer clientBuffer = glext::eglGetNativeClientBufferANDROID(buffer_);
    if (!clientBuffer) {
        XR_ERROR("bindTexture failed: clientBuffer null");
        return false;
    }
    ...
    if (eglImage_ == EGL_NO_IMAGE_KHR) {
        XR_ERROR("bindTexture failed: eglCreateImageKHR null: %s", getEGLError());
        return false;
    }
    XR_DEBUG("texId:%d,%s", texId, ismultiview ? "multiview" : "no multiview");
    GLenum texttype = ismultiview ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
    GL(glBindTexture(texttype, texId));
    GL(glext::glEGLImageTargetTexture2DOES(texttype, (GLeglImageOES) eglImage_));
...
jobject SharedTexture::getBufferJObject(JNIEnv *env) const {
    if (!AVAILABLE) {
        XR_ERROR("getBufferJObject null: not AVAILABLE");
        return nullptr;
    }
    if (!buffer_) {
        XR_ERROR("getBufferJObject null: buffer_ null");
        return nullptr;
    }
...
int SharedTexture::createEGLFence() {
    XR_LOGGER_NO_INSTANCE(logger);
    if (!AVAILABLE) {
        XR_ERROR("createEGLFence null: not AVAILABLE");
        return EGL_NO_NATIVE_FENCE_FD_ANDROID;
    }
    GL();
    EGLDisplay display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    XR_DEBUG("EGLDisplay:%p", display);
    EGLSyncKHR eglSync = glext::eglCreateSyncKHR(display, EGL_SYNC_NATIVE_FENCE_ANDROID, nullptr);
    if (eglSync == EGL_NO_SYNC_KHR) {
        XR_ERROR("createEGLFence null: eglCreateSyncKHR null: %s", getEGLError());
        return EGL_NO_NATIVE_FENCE_FD_ANDROID;
    }
    ...
    if (fenceFd == EGL_NO_NATIVE_FENCE_FD_ANDROID) {
        XR_ERROR("createEGLFence null: eglDupNativeFenceFDANDROID error: %s", getEGLError());
    }
    GL();
    return fenceFd;
}
...
bool SharedTexture::waitEGLFence(int fenceFd) {
    if (!AVAILABLE) {
        XR_ERROR("waitEGLFence failed: not AVAILABLE");
        return false;
    }
    ...
    if (eglSync == EGL_NO_SYNC_KHR) {
        XR_ERROR("waitEGLFence failed: eglCreateSyncKHR null: %s", getEGLError());
        close(fenceFd);
        return false;
    }
    ...
    if (success == EGL_FALSE) {
        XR_ERROR("waitEGLFence failed: eglWaitSyncKHR fail: %s", getEGLError());
        return false;
    }
